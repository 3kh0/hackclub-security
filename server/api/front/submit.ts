...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    await pg.query(
      ...
      [reportId, new Date().toISOString(), name, email, region, affectedPrograms, title, description, cvssScore, severity, 'open', [email]]
    );

    // Send confirmation email to reporter
    await email(email, `Thank you for submitting your report to the Hack Club Security program. Your report ID is ${reportId}. We will send any updates to your email address.`);
